...
} from '/var/task/microsaas-projects/parislocal/.netlify/dist/run/handlers/request-context.cjs'
import { getTracer, withActiveSpan } from '/var/task/microsaas-projects/parislocal/.netlify/dist/run/handlers/tracer.cjs'

process.chdir('/var/task/microsaas-projects/parislocal')
  ...
  const handlerResponse = await runWithRequestContext(requestContext, () => {
    return withActiveSpan(tracer, 'Next.js Server Handler', async (span) => {
      span?.setAttributes({
        ...
        monorepo: true,
        cwd: '/var/task/microsaas-projects/parislocal',
      })
      if (!cachedHandler) {
        const { default: handler } = await import('/var/task/microsaas-projects/parislocal/.netlify/dist/run/handlers/server.js')
